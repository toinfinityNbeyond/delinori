...
<script>
    ...
    document.querySelector("#uploadBtn").addEventListener("click", (e) => {
        ...
        axios.post("/saleboard/upload", formData, headerObj).then((response) => {
            ...
            for (let i = 0; i < arr.length; i++) {
                ...
                if (image) {
                    str += `<div data-uuid='\${uuid}' data-filename='\${fileName}' data-uploadpath='\${uploadPath}' data-image='\${image}'>
                            <img src="/saleboard/viewFile?file=\${thumbnail}"/>
                            <button onclick="javascript:removeFile('\${fileLink}',this)">x</button></div>`
                } else {
                    str += `<div data-uuid='\${uuid}' data-filename='\${fileName}' data-uploadpath='\${uploadPath}' data-image='\${image}'>
                            <a href="/saleboard/downFile?file=\${fileLink}">\${fileName}</a><button onclick="javascript:removeFile('\${fileLink}',this)">x</button></div>`
                }
    ...
    function removeFile(fileLink, ele) {
        ...
        axios.post("/saleboard/removeFile", {fileName: fileLink}).then(response => {
